...
lectureController.updateLecture = async(req, res) => {
    try {
        ...
        const {name, img, ctgry, desc, price, txtbkStck, fileTxtbk, status, items, dscnt, dscntRt} = req.body;
        const {userId} = req;
        const lecture = await Lecture.findByIdAndUpdate({_id: lectureId}, {name, img, ctgry, desc, price, txtbkStck, fileTxtbk, status, items, dscnt, dscntRt, userId}, {new: true});
